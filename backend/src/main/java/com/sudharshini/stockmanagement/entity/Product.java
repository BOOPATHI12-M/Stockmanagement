...
public class Product {
    ...
    /**
     * Check if product is near expiry (within 5 days)
     */
    public boolean isNearExpiry() {
        if (expiryDate == null) return false;
        LocalDate today = LocalDate.now();
        LocalDate fiveDaysFromNow = today.plusDays(5);
        // Product is near expiry if it expires today or within the next 5 days (and hasn't expired yet)
        return (expiryDate.isEqual(today) || expiryDate.isAfter(today)) 
            && (expiryDate.isBefore(fiveDaysFromNow) || expiryDate.isEqual(fiveDaysFromNow));
    }
